...
def numba_DFS(recursion, unassociated_measurements, \
                level, association, compat_boundaries, \
                boundary_points, boundaries_taken, L, h, scan_data, \
                    best_association, bnbJN, chi2table):
    recursion +=1
    if recursion >= 200:
    ...
    for next_boundary in avail_boundaries:
        ...
        if isValidBoundary and level < len(unassociated_measurements):
            ...
            update = False
            if joint_compat and num_associated >= bnbJN[0]:
                if num_associated == bnbJN[0]:
                    if JNIS <= bnbJN[1] and JNIS != 0:
                        update = True
                else:
                    update = True
            if update:
                bnbJN[1] = JNIS
                bnbJN[0] = num_associated
                best_association[:] = np.copy(test_association)
            if joint_compat and level+1 < len(unassociated_measurements):
                ...
                numba_DFS(recursion, unassociated_measurements, level+1, \
                            test_association, compat_boundaries, boundary_points, boundaries_taken, \
                            L, h, scan_data, best_association, bnbJN, chi2table)
    return bnbJN, best_association          
@jit(nopython=True)
...
class JCBB:
    def __init__(self):
        ...
        bnbJN = np.array([best_num_associated, best_JNIS], dtype=np.float64)
        numba_DFS(0, unassociated_measurements, \
                0, minimal_association, compat_boundaries, \
                boundary_points, boundaries_taken, L, h, scan_data, \
                best_association, bnbJN, self.chi2table)
        print("Loaded.")
    ...
    def branch_and_bound(self, unassociated_measurements, minimal_association, compat_boundaries, boundary_points):
        ...
        bnbJN = np.array([self.best_num_associated,self.best_JNIS], dtype=np.float64)
        breakpoint()
        try:
            #print("DFS begin.")
            # self.DFS(0, minimal_association, compat_boundaries, boundary_points, boundaries_taken)
            bnbJN, best_association = numba_DFS(0, self.unassociated_measurements, \
                0, minimal_association, compat_boundaries, \
                boundary_points, boundaries_taken, self.L, self.h, self.scan_data, \
                    self.best_association, bnbJN, self.chi2table)
            
           
            self.best_JNIS = bnbJN[1]
            self.best_num_associated = bnbJN[0]
            print(bnbJN)
            print(best_association)
