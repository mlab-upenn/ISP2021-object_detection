...
##CUSTOM IMPORTS
from perception.cleanupstates import CleanUpStates
from perception.cluster import Cluster
from perception.coarse_association import Coarse_Association
...
if __name__ == '__main__':
    ...
    while not done:
        ...
        noise = rng.normal(0., 0.05, size=num_beams)
        Q_s = current_scan
        ...
        cleaned = cleanup.run(Q_s_cart, agent_x, agent_y, lidar_range=30.0)
        ...
        # plt.scatter(cleaned_noise[:,0],cleaned_noise[:,1])
        # plt.show()
        with open('lidar'+str(i)+'.npy', 'wb') as f:
            np.save(f, cleaned)

        with open('static_background'+str(i)+'.npy', 'wb') as f:
            np.save(f, static_background)

        with open('dynamic_tracks'+str(i)+'.npy', 'wb') as f:
            np.save(f, dynamic)
        i =+1
        ...
        plt.show()
        C = cl.cluster(static_background)
        # with open('static_background.npy', 'wb') as f:
        #     np.save(f, C)
        # with open('C2.npy', 'wb') as f:
        #     np.save(f, C)
        # for key in C.keys():
        #     P = cleaned_noise[C[key]]
        #     plt.scatter(P[:,0], P[:,1])
        # plt.show()
        ca = Coarse_Association(C)
        dynamic_tracks_dict = createDictDynamicTrack(1, dynamic)
        for key in dynamic_tracks_dict.keys():
            P = cleaned[dynamic_tracks_dict[key]]
            plt.scatter(P[:,0], P[:,1])
        plt.show()
        A, A_d, new_tracks = ca.run(cleaned, static_background, dynamic, dynamic_tracks_dict)
        for key in A.keys():
            P = cleaned[A[key]]
        ...
        #     P_new = cleaned[new_tracks[key]]
        #     clusters_new = plt.scatter(P_new[:,0], P_new[:,1], marker = 'o')
        car = plt.scatter(agent_x, agent_y)
        # plt.legend((car, clusters_s, clusters_d),("car", "assigned static background", "assigned dynamic tracks"))
        plt.show()
